...
* 호스팅 요구 사항: 자동 도구 업데이트를 위해 클러스터 연결 및 인터넷 액세스가 가능한 위치에 VM을 배치하십시오. 자동 업데이트를 수행하려면 해당 머신이 HTTPS(443)를 통해 다음 엔드포인트에 액세스할 수 있어야 합니다.
+
...
또한 가상 머신이 HTTPS를 통해 텔레메트리 엔드포인트(+https://support.netapp.com+ 및 +https://api.activeiq.netapp.com+)에 연결할 수 있는지 확인하여 NetApp이 업그레이드 계획에 대한 AutoSupport 정보를 수신할 수 있도록 하십시오.
